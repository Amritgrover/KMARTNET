// ============================================================
// KMART → Vision Bridge (Salesman Orders Only)
...
function log(msg) {
  console.log(`[Salesman Bridge] [${new Date().toLocaleString('en-IN')}] ${msg}`);
}
...
log('🚀 KMART-Vision Salesman Bridge started');
...
sql.connect(CONFIG.sqlServer).then(pool => {
  ...
  async function syncOrders() {
    log('Checking Supabase for pending salesman orders...');
    try {
      ...
      if (!orders?.length) { log('No pending salesman orders.'); return; }

      log(`Found ${orders.length} salesman order(s) — syncing to Vision...`);
