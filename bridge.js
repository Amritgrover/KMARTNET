// ============================================================
// KMART → Vision Bridge — FINAL VERSION
...
// ============================================================
// CONFIG — FILL IN YOUR SUPABASE SERVICE ROLE KEY ONLY
// ============================================================
const CONFIG = {
  supabase: {
  ...
  sqlServer: {
    ...
    user: 'sa',
    password: 'kmart@123',
    requestTimeout: 60000,      // ← add this (60 seconds)
...
async function insertBillRow(pool, p) {
  ...
  // Fix: Swapped PKG and NUGS.
  // In Vision UI, "Pkg." column maps to database "PKG" column, which should receive pack/case size (p.nugs = 120).
  // "Box" column maps to database "NUGS" column, which should receive the box count (p.pkg = 1).
...
// ============================================================
// MAIN SYNC — runs every 30 seconds
// ============================================================
async function syncOrders() {
  log('Checking Supabase for pending orders...');
  let pool;

  try {
    // items is jsonb: [{id, qty, name, price, discount}]
    // qty = total PCS — split into cases + loose using Vision PACK1
    ...
    if (!orders?.length) { log('No pending orders.'); return; }

    log(`Found ${orders.length} order(s) — syncing to Vision...`);
    ...
    for (const order of orders) {
      try {
        ...
        if (!visCustId) {
          throw new Error(`Vision Customer ID not mapped — set vision_customer_id on this customer in Supabase`);
        }

        const items = order.items; // already parsed jsonb

        // Batch fetch all product Vision IDs in one query
        ...
        // Lock BILL table and get next safe IDs
      
        const { next_uncode, next_bno } = await getNextIds(pool);
        ...
        for (const item of items) {
          ...
          // Get item master from Vision — PACK1 = case size, SGST+CGST = GST rate
          const vItem = await getVisionItem(pool, prodInfo.vision_product_id);
          if (!vItem) {
            throw new Error(`ITCODE ${prodInfo.vision_product_id} not found in Vision item table`);
          }

          const caseSize = vItem.PACK1 || 1;
          const gstRate  = (vItem.SGST || 0) + (vItem.CGST || 0);

          // Split total PCS into full cases + remaining loose pieces
          ...
          await insertBillRow(pool, {
            ...
            gcode:     vItem.GCODE || 0,
            smcode:    visSalesId,
            itcode:    prodInfo.vision_product_id,
            bno:       next_bno,
...
log('🚀 KMART-Vision Bridge started');
    ...
    try {
      ...
      for (const order of orders) {
        try {
          ...
          // Get next bill and row IDs — no transaction needed (single user)
